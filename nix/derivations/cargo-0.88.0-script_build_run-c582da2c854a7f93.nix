...
{ pkgs, fn, cargo, rustc, deps, project_root, cargo-0_88_0-script_build-2f8a8657b7eefbe7 }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "cargo-0_88_0-script_build_run-c582da2c854a7f93";
    meta.cargo_crate_info = {
      name = "cargo";
      version = "0.88.0";
      crate_hash = "c582da2c854a7f93";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [cargo-0_88_0-script_build-2f8a8657b7eefbe7];
    passthru.rust_script_build_run = [curl-sys-0_4_80_plus_curl-8_12_1-e26ceb4086d88aa5 libgit2-sys-0_18_0_plus_1_9_0-7c92f3a3d35092c8];
 ...
 src = pkgs.lib.fileset.toSource rec {
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ cargo-0_88_0-script_build-2f8a8657b7eefbe7 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/cargo-0.88.0-script_build_run-c582da2c854a7f93.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/cargo-0.88.0-script_build_run-c582da2c854a7f93.nix':" $build_script_build_exit_value $build_script_build_output_lines
