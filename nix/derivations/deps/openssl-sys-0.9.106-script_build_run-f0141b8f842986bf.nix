...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "openssl-sys-0_9_106-script_build_run-f0141b8f842986bf";
    meta.cargo_crate_info = {
      name = "openssl-sys";
      version = "0.9.106";
      crate_hash = "f0141b8f842986bf";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [openssl-sys-0_9_106-script_build-00ef8a6cf885066c];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ openssl-sys-0_9_106-script_build-00ef8a6cf885066c }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/openssl-sys-0.9.106-script_build_run-f0141b8f842986bf.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/openssl-sys-0.9.106-script_build_run-f0141b8f842986bf.nix':" $build_script_build_exit_value $build_script_build_output_lines
