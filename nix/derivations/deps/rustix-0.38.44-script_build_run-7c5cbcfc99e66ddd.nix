...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "rustix-0_38_44-script_build_run-7c5cbcfc99e66ddd";
    meta.cargo_crate_info = {
      name = "rustix";
      version = "0.38.44";
      crate_hash = "7c5cbcfc99e66ddd";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [rustix-0_38_44-script_build-1617132ebd0bc328];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ rustix-0_38_44-script_build-1617132ebd0bc328 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/rustix-0.38.44-script_build_run-7c5cbcfc99e66ddd.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/rustix-0.38.44-script_build_run-7c5cbcfc99e66ddd.nix':" $build_script_build_exit_value $build_script_build_output_lines
