...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m curl-0_4_47-script_build_run-ba10f3f61d2b9233"
      echo "@cargo { \"type\":0, \"crate_name\":\"curl\", \"id\":\"curl-0_4_47-script_build_run-ba10f3f61d2b9233\" }"
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "curl" \
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/curl-0.4.47-script_build_run-ba10f3f61d2b9233.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "curl" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/curl-0.4.47-script_build_run-ba10f3f61d2b9233.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"curl\", \"exit_code\": 0, \"messages\": []}"
    '';
