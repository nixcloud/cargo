...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "parking_lot_core-0_9_10-script_build_run-323cca02bf1fcaa8";
    meta.cargo_crate_info = {
      name = "parking_lot_core";
      version = "0.9.10";
      crate_hash = "323cca02bf1fcaa8";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [parking_lot_core-0_9_10-script_build-a14671daa7ac8ab9];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ parking_lot_core-0_9_10-script_build-a14671daa7ac8ab9 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/parking_lot_core-0.9.10-script_build_run-323cca02bf1fcaa8.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/parking_lot_core-0.9.10-script_build_run-323cca02bf1fcaa8.nix':" $build_script_build_exit_value $build_script_build_output_lines
