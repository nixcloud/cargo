...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "ahash-0_8_11-script_build_run-eacbc68f79a20c26";
    meta.cargo_crate_info = {
      name = "ahash";
      version = "0.8.11";
      crate_hash = "eacbc68f79a20c26";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [ahash-0_8_11-script_build-abac3b41dce7b5e7];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ ahash-0_8_11-script_build-abac3b41dce7b5e7 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/ahash-0.8.11-script_build_run-eacbc68f79a20c26.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/ahash-0.8.11-script_build_run-eacbc68f79a20c26.nix':" $build_script_build_exit_value $build_script_build_output_lines
