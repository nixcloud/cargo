...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "lock_api-0_4_12-script_build_run-e5bf3553f6529fb8";
    meta.cargo_crate_info = {
      name = "lock_api";
      version = "0.4.12";
      crate_hash = "e5bf3553f6529fb8";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [lock_api-0_4_12-script_build-32f26ccb87f26f79];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ lock_api-0_4_12-script_build-32f26ccb87f26f79 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/lock_api-0.4.12-script_build_run-e5bf3553f6529fb8.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/lock_api-0.4.12-script_build_run-e5bf3553f6529fb8.nix':" $build_script_build_exit_value $build_script_build_output_lines
