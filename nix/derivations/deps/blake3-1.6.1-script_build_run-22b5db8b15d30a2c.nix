...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m blake3-1_6_1-script_build_run-22b5db8b15d30a2c"
      echo "@cargo { \"type\":0, \"crate_name\":\"blake3\", \"id\":\"blake3-1_6_1-script_build_run-22b5db8b15d30a2c\" }"
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              ...
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/blake3-1.6.1-script_build_run-22b5db8b15d30a2c.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "blake3" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/blake3-1.6.1-script_build_run-22b5db8b15d30a2c.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"blake3\", \"exit_code\": 0, \"messages\": []}"
    '';
