...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "libsqlite3-sys-0_31_0-script_build_run-2e938fb39223d7f4";
    meta.cargo_crate_info = {
      name = "libsqlite3-sys";
      version = "0.31.0";
      crate_hash = "2e938fb39223d7f4";
    };
    buildInputs = [] ++ fn.inject meta.cargo_crate_info;
    passthru.rust_crate_libraries = [];
    passthru.rust_crate_parent = [libsqlite3-sys-0_31_0-script_build-ccf5f6176bdb99ad];
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m libsqlite3-sys-0_31_0-script_build_run-2e938fb39223d7f4"
      echo "@cargo { \"type\":0, \"crate_name\":\"libsqlite3-sys\", \"id\":\"libsqlite3-sys-0_31_0-script_build_run-2e938fb39223d7f4\" }"
      ...
      set -x +e
      ${ libsqlite3-sys-0_31_0-script_build-ccf5f6176bdb99ad }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "libsqlite3-sys" \
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libsqlite3-sys-0.31.0-script_build_run-2e938fb39223d7f4.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "libsqlite3-sys" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libsqlite3-sys-0.31.0-script_build_run-2e938fb39223d7f4.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"libsqlite3-sys\", \"exit_code\": 0, \"messages\": []}"
    '';
