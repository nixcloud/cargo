...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "proc-macro2-1_0_93-script_build_run-63c84d0e53e082e9";
    meta.cargo_crate_info = {
      name = "proc-macro2";
      version = "1.0.93";
      crate_hash = "63c84d0e53e082e9";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [proc-macro2-1_0_93-script_build-9d8a03f0bd260577];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ proc-macro2-1_0_93-script_build-9d8a03f0bd260577 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/proc-macro2-1.0.93-script_build_run-63c84d0e53e082e9.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/proc-macro2-1.0.93-script_build_run-63c84d0e53e082e9.nix':" $build_script_build_exit_value $build_script_build_output_lines
