...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "thiserror-2_0_11-script_build_run-8296f45a178e6d2d";
    meta.cargo_crate_info = {
      name = "thiserror";
      version = "2.0.11";
      crate_hash = "8296f45a178e6d2d";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [thiserror-2_0_11-script_build-fba4a45f74aabdd3];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ thiserror-2_0_11-script_build-fba4a45f74aabdd3 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/thiserror-2.0.11-script_build_run-8296f45a178e6d2d.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/thiserror-2.0.11-script_build_run-8296f45a178e6d2d.nix':" $build_script_build_exit_value $build_script_build_output_lines
