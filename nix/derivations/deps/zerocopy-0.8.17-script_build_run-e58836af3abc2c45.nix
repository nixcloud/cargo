...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "zerocopy-0_8_17-script_build_run-e58836af3abc2c45";
    meta.cargo_crate_info = {
      name = "zerocopy";
      version = "0.8.17";
      crate_hash = "e58836af3abc2c45";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [zerocopy-0_8_17-script_build-e99c5b6fa21fd0f7];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ zerocopy-0_8_17-script_build-e99c5b6fa21fd0f7 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/zerocopy-0.8.17-script_build_run-e58836af3abc2c45.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/zerocopy-0.8.17-script_build_run-e58836af3abc2c45.nix':" $build_script_build_exit_value $build_script_build_output_lines
