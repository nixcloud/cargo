...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "num-traits-0_2_19-script_build_run-751c349c04aeff9e";
    meta.cargo_crate_info = {
      name = "num-traits";
      version = "0.2.19";
      crate_hash = "751c349c04aeff9e";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [num-traits-0_2_19-script_build-1439b8816ccd5d7f];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ num-traits-0_2_19-script_build-1439b8816ccd5d7f }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/num-traits-0.2.19-script_build_run-751c349c04aeff9e.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/num-traits-0.2.19-script_build_run-751c349c04aeff9e.nix':" $build_script_build_exit_value $build_script_build_output_lines
