...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "serde_json-1_0_139-script_build_run-0c75c9c70c6b060e";
    meta.cargo_crate_info = {
      name = "serde_json";
      version = "1.0.139";
      crate_hash = "0c75c9c70c6b060e";
    };
    buildInputs = [] ++ fn.inject meta.cargo_crate_info;
    passthru.rust_crate_libraries = [];
    passthru.rust_crate_parent = [serde_json-1_0_139-script_build-fb6175468e374361];
    ...
    CARGO_CFG_FEATURE = "default,raw_value,std";
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m serde_json-1_0_139-script_build_run-0c75c9c70c6b060e"
      echo "@cargo { \"type\":0, \"crate_name\":\"serde_json\", \"id\":\"serde_json-1_0_139-script_build_run-0c75c9c70c6b060e\" }"
      ...
      set -x +e
      ${ serde_json-1_0_139-script_build-fb6175468e374361 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              ...
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/serde_json-1.0.139-script_build_run-0c75c9c70c6b060e.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "serde_json" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/serde_json-1.0.139-script_build_run-0c75c9c70c6b060e.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"serde_json\", \"exit_code\": 0, \"messages\": []}"
    '';
