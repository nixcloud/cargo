...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "im-rc-15_1_0-script_build_run-d8792684cc68fdb8";
    meta.cargo_crate_info = {
      name = "im-rc";
      version = "15.1.0";
      crate_hash = "d8792684cc68fdb8";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [im-rc-15_1_0-script_build-107ebc29aa288c61];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ im-rc-15_1_0-script_build-107ebc29aa288c61 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/im-rc-15.1.0-script_build_run-d8792684cc68fdb8.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/im-rc-15.1.0-script_build_run-d8792684cc68fdb8.nix':" $build_script_build_exit_value $build_script_build_output_lines
