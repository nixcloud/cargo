...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "lock_api-0_4_12-script_build_run-a80fcfc254bcd832";
    meta.cargo_crate_info = {
      name = "lock_api";
      version = "0.4.12";
      crate_hash = "a80fcfc254bcd832";
    };
    buildInputs = [] ++ fn.inject meta.cargo_crate_info;
    passthru.rust_crate_libraries = [];
    passthru.rust_crate_parent = [lock_api-0_4_12-script_build-d0a80824f419daa1];
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m lock_api-0_4_12-script_build_run-a80fcfc254bcd832"
      echo "@cargo { \"type\":0, \"crate_name\":\"lock_api\", \"id\":\"lock_api-0_4_12-script_build_run-a80fcfc254bcd832\" }"
      ...
      set -x +e
      ${ lock_api-0_4_12-script_build-d0a80824f419daa1 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "lock_api" \
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/lock_api-0.4.12-script_build_run-a80fcfc254bcd832.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "lock_api" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/lock_api-0.4.12-script_build_run-a80fcfc254bcd832.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"lock_api\", \"exit_code\": 0, \"messages\": []}"
    '';
