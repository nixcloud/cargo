...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "thiserror-2_0_11-script_build_run-d6ba5533733cef05";
    meta.cargo_crate_info = {
      name = "thiserror";
      version = "2.0.11";
      crate_hash = "d6ba5533733cef05";
      type = "(build.rs run)";
    };
    ...
    buildPhase = ''
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/thiserror-2.0.11-script_build_run-d6ba5533733cef05.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/thiserror-2.0.11-script_build_run-d6ba5533733cef05.nix':" $build_script_build_exit_value $build_script_build_output_lines
