...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m semver-1_0_25-script_build_run-e61c72ddfe9a1461"
      echo "@cargo { \"type\":0, \"crate_name\":\"semver\", \"id\":\"semver-1_0_25-script_build_run-e61c72ddfe9a1461\" }"
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              ...
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/semver-1.0.25-script_build_run-e61c72ddfe9a1461.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "semver" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/semver-1.0.25-script_build_run-e61c72ddfe9a1461.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"semver\", \"exit_code\": 0, \"messages\": []}"
    '';
