...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "libssh2-sys-0_3_1-script_build_run-da060cf47d1b21a6";
    meta.cargo_crate_info = {
      name = "libssh2-sys";
      version = "0.3.1";
      crate_hash = "da060cf47d1b21a6";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [libssh2-sys-0_3_1-script_build-6dcd35b2650a9ca9];
    passthru.rust_script_build_run = [libz-sys-1_1_21-8eb7d635fee6f8cb openssl-sys-0_9_106-fe430717cd107543];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ libssh2-sys-0_3_1-script_build-6dcd35b2650a9ca9 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/libssh2-sys-0.3.1-script_build_run-da060cf47d1b21a6.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/libssh2-sys-0.3.1-script_build_run-da060cf47d1b21a6.nix':" $build_script_build_exit_value $build_script_build_output_lines
