...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "num-traits-0_2_19-script_build_run-7e89687594b4e195";
    meta.cargo_crate_info = {
      name = "num-traits";
      version = "0.2.19";
      crate_hash = "7e89687594b4e195";
    };
    buildInputs = [] ++ fn.inject meta.cargo_crate_info;
    passthru.rust_crate_libraries = [];
    passthru.rust_crate_parent = [num-traits-0_2_19-script_build-34b314758d860194];
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m num-traits-0_2_19-script_build_run-7e89687594b4e195"
      echo "@cargo { \"type\":0, \"crate_name\":\"num-traits\", \"id\":\"num-traits-0_2_19-script_build_run-7e89687594b4e195\" }"
      ...
      set -x +e
      ${ num-traits-0_2_19-script_build-34b314758d860194 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              ...
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/num-traits-0.2.19-script_build_run-7e89687594b4e195.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "num-traits" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/num-traits-0.2.19-script_build_run-7e89687594b4e195.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"num-traits\", \"exit_code\": 0, \"messages\": []}"
    '';
