...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "libsqlite3-sys-0_31_0-script_build_run-c31a64d006305f9c";
    meta.cargo_crate_info = {
      name = "libsqlite3-sys";
      version = "0.31.0";
      crate_hash = "c31a64d006305f9c";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [libsqlite3-sys-0_31_0-script_build-65af74c81b4b60aa];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ libsqlite3-sys-0_31_0-script_build-65af74c81b4b60aa }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/libsqlite3-sys-0.31.0-script_build_run-c31a64d006305f9c.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/libsqlite3-sys-0.31.0-script_build_run-c31a64d006305f9c.nix':" $build_script_build_exit_value $build_script_build_output_lines
