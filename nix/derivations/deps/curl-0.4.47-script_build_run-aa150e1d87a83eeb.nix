...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "curl-0_4_47-script_build_run-aa150e1d87a83eeb";
    meta.cargo_crate_info = {
      name = "curl";
      version = "0.4.47";
      crate_hash = "aa150e1d87a83eeb";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [curl-0_4_47-script_build-6927a4ffca2b77e3];
    passthru.rust_script_build_run = [curl-sys-0_4_80_plus_curl-8_12_1-e26ceb4086d88aa5 openssl-sys-0_9_106-fe430717cd107543];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ curl-0_4_47-script_build-6927a4ffca2b77e3 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/curl-0.4.47-script_build_run-aa150e1d87a83eeb.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/curl-0.4.47-script_build_run-aa150e1d87a83eeb.nix':" $build_script_build_exit_value $build_script_build_output_lines
