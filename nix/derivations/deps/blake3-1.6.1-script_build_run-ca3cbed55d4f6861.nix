...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "blake3-1_6_1-script_build_run-ca3cbed55d4f6861";
    meta.cargo_crate_info = {
      name = "blake3";
      version = "1.6.1";
      crate_hash = "ca3cbed55d4f6861";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [blake3-1_6_1-script_build-be65d8b05892796a];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ blake3-1_6_1-script_build-be65d8b05892796a }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/blake3-1.6.1-script_build_run-ca3cbed55d4f6861.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/blake3-1.6.1-script_build_run-ca3cbed55d4f6861.nix':" $build_script_build_exit_value $build_script_build_output_lines
