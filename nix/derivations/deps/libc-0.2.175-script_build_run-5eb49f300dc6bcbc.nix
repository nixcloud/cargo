...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m libc-0_2_175-script_build_run-5eb49f300dc6bcbc"
      echo "@cargo { \"type\":0, \"crate_name\":\"libc\", \"id\":\"libc-0_2_175-script_build_run-5eb49f300dc6bcbc\" }"
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "libc" \
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libc-0.2.175-script_build_run-5eb49f300dc6bcbc.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "libc" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libc-0.2.175-script_build_run-5eb49f300dc6bcbc.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"libc\", \"exit_code\": 0, \"messages\": []}"
    '';
