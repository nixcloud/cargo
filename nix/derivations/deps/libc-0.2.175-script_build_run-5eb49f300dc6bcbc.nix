...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "libc-0_2_175-script_build_run-5eb49f300dc6bcbc";
    meta.cargo_crate_info = {
      name = "libc";
      version = "0.2.175";
      crate_hash = "5eb49f300dc6bcbc";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [libc-0_2_175-script_build-8e5406a56535c6d3];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ libc-0_2_175-script_build-8e5406a56535c6d3 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libc-0.2.175-script_build_run-5eb49f300dc6bcbc.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libc-0.2.175-script_build_run-5eb49f300dc6bcbc.nix':" $build_script_build_exit_value $build_script_build_output_lines
