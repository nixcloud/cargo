...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "serde_json-1_0_139-script_build_run-3855ca059ae92bb9";
    meta.cargo_crate_info = {
      name = "serde_json";
      version = "1.0.139";
      crate_hash = "3855ca059ae92bb9";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [serde_json-1_0_139-script_build-45e4b2f960af770d];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ serde_json-1_0_139-script_build-45e4b2f960af770d }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/serde_json-1.0.139-script_build_run-3855ca059ae92bb9.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/serde_json-1.0.139-script_build_run-3855ca059ae92bb9.nix':" $build_script_build_exit_value $build_script_build_output_lines
