...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "getrandom-0_3_1-script_build_run-cb15c936ded7c8a6";
    meta.cargo_crate_info = {
      name = "getrandom";
      version = "0.3.1";
      crate_hash = "cb15c936ded7c8a6";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [getrandom-0_3_1-script_build-394ed08c28b050f0];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ getrandom-0_3_1-script_build-394ed08c28b050f0 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/getrandom-0.3.1-script_build_run-cb15c936ded7c8a6.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/getrandom-0.3.1-script_build_run-cb15c936ded7c8a6.nix':" $build_script_build_exit_value $build_script_build_output_lines
