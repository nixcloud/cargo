...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "curl-sys-0_4_80_plus_curl-8_12_1-script_build_run-3b4d64c654555321";
    meta.cargo_crate_info = {
      name = "curl-sys";
      version = "0.4.80+curl-8.12.1";
      crate_hash = "3b4d64c654555321";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [curl-sys-0_4_80_plus_curl-8_12_1-script_build-995e765fec62c215];
    passthru.rust_script_build_run = [libnghttp2-sys-0_1_11_plus_1_64_0-fc24622a308fad5a libz-sys-1_1_21-8eb7d635fee6f8cb openssl-sys-0_9_106-fe430717cd107543];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ curl-sys-0_4_80_plus_curl-8_12_1-script_build-995e765fec62c215 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/curl-sys-0.4.80_plus_curl-8.12.1-script_build_run-3b4d64c654555321.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/curl-sys-0.4.80_plus_curl-8.12.1-script_build_run-3b4d64c654555321.nix':" $build_script_build_exit_value $build_script_build_output_lines
