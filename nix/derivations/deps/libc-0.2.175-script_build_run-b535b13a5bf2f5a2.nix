...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "libc-0_2_175-script_build_run-b535b13a5bf2f5a2";
    meta.cargo_crate_info = {
      name = "libc";
      version = "0.2.175";
      crate_hash = "b535b13a5bf2f5a2";
      type = "(build.rs run)";
    };
    ...
    buildPhase = ''
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/libc-0.2.175-script_build_run-b535b13a5bf2f5a2.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/libc-0.2.175-script_build_run-b535b13a5bf2f5a2.nix':" $build_script_build_exit_value $build_script_build_output_lines
