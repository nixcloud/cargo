...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m ahash-0_8_11-script_build_run-e66d9dc18aec0370"
      echo "@cargo { \"type\":0, \"crate_name\":\"ahash\", \"id\":\"ahash-0_8_11-script_build_run-e66d9dc18aec0370\" }"
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              ...
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/ahash-0.8.11-script_build_run-e66d9dc18aec0370.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "ahash" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/ahash-0.8.11-script_build_run-e66d9dc18aec0370.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"ahash\", \"exit_code\": 0, \"messages\": []}"
    '';
