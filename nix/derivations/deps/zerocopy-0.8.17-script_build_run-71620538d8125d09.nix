...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m zerocopy-0_8_17-script_build_run-71620538d8125d09"
      echo "@cargo { \"type\":0, \"crate_name\":\"zerocopy\", \"id\":\"zerocopy-0_8_17-script_build_run-71620538d8125d09\" }"
      ...
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              ...
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/zerocopy-0.8.17-script_build_run-71620538d8125d09.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "zerocopy" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/zerocopy-0.8.17-script_build_run-71620538d8125d09.nix':" \
      ...
      echo "@cargo {\"type\": 3, \"crate_name\": \"zerocopy\", \"exit_code\": 0, \"messages\": []}"
    '';
