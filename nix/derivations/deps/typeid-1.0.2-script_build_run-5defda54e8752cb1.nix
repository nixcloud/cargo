...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "typeid-1_0_2-script_build_run-5defda54e8752cb1";
    meta.cargo_crate_info = {
      name = "typeid";
      version = "1.0.2";
      crate_hash = "5defda54e8752cb1";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [typeid-1_0_2-script_build-7d9a84fcce5c617d];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ typeid-1_0_2-script_build-7d9a84fcce5c617d }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/typeid-1.0.2-script_build_run-5defda54e8752cb1.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/typeid-1.0.2-script_build_run-5defda54e8752cb1.nix':" $build_script_build_exit_value $build_script_build_output_lines
