...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "thiserror-1_0_69-script_build_run-ac159c9a9043953b";
    meta.cargo_crate_info = {
      name = "thiserror";
      version = "1.0.69";
      crate_hash = "ac159c9a9043953b";
    };
    buildInputs = [] ++ fn.inject meta.cargo_crate_info;
    passthru.rust_crate_libraries = [];
    passthru.rust_crate_parent = [thiserror-1_0_69-script_build-6cadb049729033d9];
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m thiserror-1_0_69-script_build_run-ac159c9a9043953b"
      echo "@cargo { \"type\":0, \"crate_name\":\"thiserror\", \"id\":\"thiserror-1_0_69-script_build_run-ac159c9a9043953b\" }"
      ...
      set -x +e
      ${ thiserror-1_0_69-script_build-6cadb049729033d9 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "thiserror" \
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/thiserror-1.0.69-script_build_run-ac159c9a9043953b.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "thiserror" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/thiserror-1.0.69-script_build_run-ac159c9a9043953b.nix':" \
