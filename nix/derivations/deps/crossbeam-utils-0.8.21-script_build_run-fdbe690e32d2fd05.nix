...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "crossbeam-utils-0_8_21-script_build_run-fdbe690e32d2fd05";
    meta.cargo_crate_info = {
      name = "crossbeam-utils";
      version = "0.8.21";
      crate_hash = "fdbe690e32d2fd05";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [crossbeam-utils-0_8_21-script_build-de090a42333daf53];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ crossbeam-utils-0_8_21-script_build-de090a42333daf53 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/crossbeam-utils-0.8.21-script_build_run-fdbe690e32d2fd05.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/crossbeam-utils-0.8.21-script_build_run-fdbe690e32d2fd05.nix':" $build_script_build_exit_value $build_script_build_output_lines
