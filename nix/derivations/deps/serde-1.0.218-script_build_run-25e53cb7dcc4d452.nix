...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "serde-1_0_218-script_build_run-25e53cb7dcc4d452";
    meta.cargo_crate_info = {
      name = "serde";
      version = "1.0.218";
      crate_hash = "25e53cb7dcc4d452";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [serde-1_0_218-script_build-ea5202fe569cdd0b];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ serde-1_0_218-script_build-ea5202fe569cdd0b }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/serde-1.0.218-script_build_run-25e53cb7dcc4d452.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/serde-1.0.218-script_build_run-25e53cb7dcc4d452.nix':" $build_script_build_exit_value $build_script_build_output_lines
