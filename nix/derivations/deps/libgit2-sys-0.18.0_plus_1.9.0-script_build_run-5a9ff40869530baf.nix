...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "libgit2-sys-0_18_0_plus_1_9_0-script_build_run-5a9ff40869530baf";
    meta.cargo_crate_info = {
      name = "libgit2-sys";
      version = "0.18.0+1.9.0";
      crate_hash = "5a9ff40869530baf";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [libgit2-sys-0_18_0_plus_1_9_0-script_build-7aac88d9d89789eb];
    passthru.rust_script_build_run = [libssh2-sys-0_3_1-6cd042864dac375d libz-sys-1_1_21-8eb7d635fee6f8cb openssl-sys-0_9_106-fe430717cd107543];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ libgit2-sys-0_18_0_plus_1_9_0-script_build-7aac88d9d89789eb }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/libgit2-sys-0.18.0_plus_1.9.0-script_build_run-5a9ff40869530baf.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/libgit2-sys-0.18.0_plus_1.9.0-script_build_run-5a9ff40869530baf.nix':" $build_script_build_exit_value $build_script_build_output_lines
