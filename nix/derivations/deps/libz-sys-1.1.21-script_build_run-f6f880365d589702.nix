...
{ pkgs, fn, cargo, rustc, deps, build_parser }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "libz-sys-1_1_21-script_build_run-f6f880365d589702";
    meta.cargo_crate_info = {
      name = "libz-sys";
      version = "1.1.21";
      crate_hash = "f6f880365d589702";
    };
    buildInputs = [] ++ fn.inject meta.cargo_crate_info;
    passthru.rust_crate_libraries = [];
    passthru.rust_crate_parent = [libz-sys-1_1_21-script_build-10baa270a769a71e];
    ...
    buildPhase = ''
      ...
      echo -e "\e[92mCompiling\e[0m libz-sys-1_1_21-script_build_run-f6f880365d589702"
      echo "@cargo { \"type\":0, \"crate_name\":\"libz-sys\", \"id\":\"libz-sys-1_1_21-script_build_run-f6f880365d589702\" }"
      ...
      set -x +e
      ${ libz-sys-1_1_21-script_build-10baa270a769a71e }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "libz-sys" \
              --arg notice "There was an error executing build_script_build in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libz-sys-1.1.21-script_build_run-f6f880365d589702.nix':" \
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          output=$(${pkgs.jq}/bin/jq -c -n \
              --arg crate_name "libz-sys" \
              --arg notice "There was an error executing build_parser in file: '/home/nixos/cargo/target/debug/nix/derivations/deps/libz-sys-1.1.21-script_build_run-f6f880365d589702.nix':" \
