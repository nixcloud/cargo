...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "signal-hook-0_3_18-script_build_run-ef232cac941a5dce";
    meta.cargo_crate_info = {
      name = "signal-hook";
      version = "0.3.18";
      crate_hash = "ef232cac941a5dce";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [signal-hook-0_3_18-script_build-c859dcc9b5669dc0];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ signal-hook-0_3_18-script_build-c859dcc9b5669dc0 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/signal-hook-0.3.18-script_build_run-ef232cac941a5dce.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/signal-hook-0.3.18-script_build_run-ef232cac941a5dce.nix':" $build_script_build_exit_value $build_script_build_output_lines
