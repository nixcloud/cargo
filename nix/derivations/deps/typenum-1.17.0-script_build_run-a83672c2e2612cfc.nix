...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "typenum-1_17_0-script_build_run-a83672c2e2612cfc";
    meta.cargo_crate_info = {
      name = "typenum";
      version = "1.17.0";
      crate_hash = "a83672c2e2612cfc";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [typenum-1_17_0-script_build-4323ae1d5b8b7451];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ typenum-1_17_0-script_build-4323ae1d5b8b7451 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/typenum-1.17.0-script_build_run-a83672c2e2612cfc.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/typenum-1.17.0-script_build_run-a83672c2e2612cfc.nix':" $build_script_build_exit_value $build_script_build_output_lines
