...
{ pkgs, fn, cargo, rustc, deps, project_root }: with deps;
  pkgs.stdenv.mkDerivation rec {
    name = "semver-1_0_25-script_build_run-683fc960cd3565a4";
    meta.cargo_crate_info = {
      name = "semver";
      version = "1.0.25";
      crate_hash = "683fc960cd3565a4";
      type = "(build.rs run)";
    };
    ...
    passthru.rust_crate_parent = [semver-1_0_25-script_build-c0920cb94340a076];
    ...
    buildPhase = ''
      ...
      set -x +e
      ${ semver-1_0_25-script_build-c0920cb94340a076 }/build_script_build > $OUT_DIR/nix/build_script_build.out 2> $build_script_build_output_lines
      build_script_build_exit_value=$?
      set +x -e
      
      if [ "$build_script_build_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_script_build in file: '/tmp/blah/derivations/deps/semver-1.0.25-script_build_run-683fc960cd3565a4.nix':" $build_script_build_exit_value $build_script_build_output_lines
      ...
      if [ "$build_parser_exit_value" -ne 0 ]; then
          print_cargo_message_type_3 "${meta.cargo_crate_info.name}" "${meta.cargo_crate_info.type}" "There was an error executing build_parser in file: '/tmp/blah/derivations/deps/semver-1.0.25-script_build_run-683fc960cd3565a4.nix':" $build_script_build_exit_value $build_script_build_output_lines
