...
impl<'a, 'gctx> BuildRunner<'a, 'gctx> {
    ...
    pub fn compile(mut self, exec: &Arc<dyn Executor>) -> CargoResult<Compilation<'gctx>> {
        ...
        //println!("compile() the initial call to bootstrap the building of the project");
        let _lock = self
            .bcx
            .gctx
            .acquire_package_cache_lock(CacheLockMode::Shared)?;
        ...
        let build_plan = self.bcx.build_config.build_plan; // this should be named output_build_plan
        self.lto = super::lto::generate(self.bcx)?;
        self.prepare_units()?;
        self.prepare()?;
    ...
    /// An abstraction of the files and directories that will be generated by the compilation.
    ...
    pub fn unit_output(&self, unit: &Unit, path: &Path) -> UnitOutput {
        let script_meta = self.find_build_script_metadata(unit);
        println!("UnitOutput: {:#?}", path.to_path_buf());
